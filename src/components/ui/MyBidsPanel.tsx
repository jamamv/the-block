...
function BidRow({ vehicleId, bidStateMap, onClose }: { vehicleId: string; bidStateMap: BidStateMap; onClose: () => void }) {
...
export function MyBidsPanel({ bidStateMap, onClose }: MyBidsPanelProps) {
    ...
    >
      ...
      {vehicleIds.length === 0 ? (
        ...
        <div className="divide-y divide-slate-100 max-h-80 overflow-y-auto">
          {vehicleIds.map((id) => (
            <BidRow key={id} vehicleId={id} bidStateMap={bidStateMap} onClose={onClose} />
          ))}
        </div>
      )}
    </div>
